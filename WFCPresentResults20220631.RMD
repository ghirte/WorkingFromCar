...
#install library mass, needed for robust regressions
...
paper "Working form self-driving car". We use GAMS to perform the simulations.
...
library(dplyr);
library(Hmisc);
#library(tidyverse);  #needed for tables
...
rm(list=ls())
## not necessary: 
## The development version from GitHub allows reading the whole 
## gdx file without defining variables in advance
##devtools::install_github("arnyeinstein/gamsr")
##library(gamsr)
##MCdatafile <- import_gdxfile("MobileWorkMonteCarlo.gdx")
...
dfmcframe <- function(gdxfile,nameparam, nameidc,asupport) {
  ...
  if (nameidc =="DE") {
    dfmc1.frame <- select(filter(df.frame, id_c == "DE"), 
                          ...
                          share_xsum,share_x,share_xo,v,vc,vo,xbar,xB,wage,wB,pb,
...
```{r dfprintMonteCarlo, message=FALSE, warning=FALSE}
...
knitr::knit("WFCPresentResults20220631.RMD")
